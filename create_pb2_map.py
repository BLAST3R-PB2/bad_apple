...
for i in range(height_pixels):
    for j in range(width_pixels):
        ...
        xml_element = f'<door uid="#d{uid}" x="{x}" y="{y}" w="{config.PIXEL_SIZE}" h="{config.PIXEL_SIZE}" tarx="0" tary="0" vis="true"/>'
        pixel_trigger_list_white = [f'actions_1_type="71" actions_1_targetA="#d{uid}" actions_1_targetB="#FFFFFF"']
        pixel_trigger_list_black = [f'actions_1_type="71" actions_1_targetA="#d{uid}" actions_1_targetB="#0"']
        
        white_trigger_actions = " ".join(pixel_trigger_list_white)
        black_trigger_actions = " ".join(pixel_trigger_list_black)

        white_trigger_element = f'<trigger uid="#{trigger_uid}" x="{x_value}" y="{y_value}" enabled="true" maxcalls="-1" {white_trigger_actions}/>'
        black_trigger_element = f'<trigger uid="#{trigger_uid + 1}" x="{x_value - 50}" y="{y_value}" enabled="true" maxcalls="-1" {black_trigger_actions}/>'
    ...
    for i in range(len(frame_data_content)):
        ...
        if frame_data_content[i] != old_frames_list[i]:
            if frame_data_content[i] == "1":  # Make it white
                timer_element = f'<timer uid="#{number_to_letters(uid)}" x="{0}" y="{0}" enabled="true" maxcalls="1" target="#{(i+1)*2-1}" delay="{timer_delay_value}"/>'
            else:  # Make it black
                timer_element = f'<timer uid="#{number_to_letters(uid)}" x="{0}" y="{0}" enabled="true" maxcalls="1" target="#{(i+1)*2}" delay="{timer_delay_value}"/>'
